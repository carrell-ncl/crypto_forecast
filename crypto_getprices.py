...
def get_hourly_data(coin, limit):
    #get real-time prices from the cryptocompare API
    call = requests.get(f'https://min-api.cryptocompare.com/data/v2/histohour?fsym=BTC&tsym=USD&limit={limit}&api_key=' + key).json()
    columns = [['Date', 'Hour', 'Open', 'High', 'Low', 'Close']]
    rows = []
    #Extract data that we want from API. Change timestamp to date.
    for values in call['Data']['Data']:
        stamps = values['time']
        date = time.strftime('%m/%d/%Y', time.localtime(stamps))
        hour = time.strftime('%H', time.localtime(stamps))
        opn = values['open']
        high = values['high']
        low = values['low']
        closing = values['close']
        rows.append((date,hour, opn, high, low, closing))
          #Create dataframe 
    filename = f'./{coin}.csv'
    with open(filename, 'w') as csvfile:
        csvwriter = csv.writer(csvfile)
